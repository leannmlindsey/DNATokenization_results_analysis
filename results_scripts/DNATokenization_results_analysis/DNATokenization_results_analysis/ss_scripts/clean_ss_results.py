...
def clean_ss_results():
    ...
    # Group by task, model, learning_rate, batch_size, rc_aug
    grouping_cols = ['task', 'model', 'learning_rate', 'batch_size', 'rc_aug']
    ...
    for combo in failed_combinations:
        task, model, lr, batch_size, rc_aug = combo
        print(f"  {task} | {model} | lr={lr} | batch_size={batch_size} | rc_aug={rc_aug}")
