...
def calculate_mamba_significance_paired_by_task():
    ...
    for task in sorted(valid_tasks):
        ...
        bpe_lr = bpe_data['learning_rate'].iloc[0] if len(bpe_data) > 0 else None
        bpe_batch = bpe_data['batch_size'].iloc[0] if len(bpe_data) > 0 else None
        char_lr = char_data['learning_rate'].iloc[0] if len(char_data) > 0 else None
        char_batch = char_data['batch_size'].iloc[0] if len(char_data) > 0 else None
        
        # Print diagnostic information
        print(f"\n{'='*60}")
        print(f"Task: {task}")
        print(f"  Mamba-bpe seeds ({len(bpe_seeds)}): {sorted(bpe_seeds)}")
        print(f"    Best LR: {bpe_lr}, Best Batch Size: {bpe_batch}")
        print(f"  Mamba-char seeds ({len(char_seeds)}): {sorted(char_seeds)}")
        print(f"    Best LR: {char_lr}, Best Batch Size: {char_batch}")
        ...
        # Limit to 10 common seeds if there are more than 10
        if len(common_seeds) > 10:
            # Sort seeds and take the first 10
            seeds_to_use = sorted(common_seeds)[:10]
            print(f"  Limiting analysis to 10 matched seeds (out of {len(common_seeds)} available)")
        else:
            seeds_to_use = sorted(common_seeds)
        ...
        task_result = {
            ...
            'category': category,
            'n_paired': len(seeds_to_use),  # Use the actual number of seeds used
